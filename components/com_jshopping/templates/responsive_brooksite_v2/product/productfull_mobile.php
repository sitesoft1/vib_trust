...
<form name="product" method="post" action="<?php print $this->action?>" enctype="multipart/form-data" autocomplete="off">
    ...
    <div class="row<?php print $row_postfix?> jshop mobile_card_view">
        <!--PRODUCT TITLE-->
        <div class="<?php print $prefix?>12">
            <h1><?php print $this->product->name?></h1>
            <?php include(dirname(__FILE__)."/ratingandhits_mobile.php");?>
    ...
    <div class="row<?php print $row_postfix?> jshop mobile_card_view">
        <!--IMAGE BLOCK-->
        <div class="<?php print $prefix?>12 image_middle">
            ...
            <div class = "jshop_img_description">
                ...
                <?php if (count($this->videos)){?>
                    <div class="border-bottom-dashed inline-block" onclick="jQuery('.mob-video').toggle();"><h4><?php print _JSHOP_TABS_VIDEO;?> &darr;</h4></div>
                    <div class="dnone mob-video">
                    <?php foreach($this->videos as $k=>$video){?>
                        <?php if ($video->video_code) { ?>
        ...
        <!--PRICE-->
        <div class="<?php print $prefix?>12 product_priceblock">
            ...
            <div class="product-full-caption">
                ...
                <?php if (count($this->attributes) || count($this->product->freeattributes)) {print "<br/>";}?>
                <!--END ATTRIBUTES-->
                
                <!--MANUFACTURER-->
                <?php if ($this->config->product_show_manufacturer && $this->product->manufacturer_info->name!="" || $this->config->product_show_manufacturer_logo && $this->product->manufacturer_info->manufacturer_logo!=""){?>
        ...
        <!--PRODUCT CAPTION-->
        <div class="<?php print $prefix?>12 product_caption">
            <!--EXTRA FIELDS-->
            ...
            <?php if(!empty($country_name) and !empty($city_name)){ ?>
                <div class="vendorinfo">
                    Регион: <span><?php echo $country_name; ?>, г. <?php echo $city_name; ?></span>
                </div>
            <?php } ?>
            
            <?php if (($this->config->product_show_manufacturer && $this->product->manufacturer_info->name!="") || $this->product->vendor_info) {print "<br/>";} ?>
